...
├── 论文v5_模板对照说明.md           稿件版式与期刊官方模板的逐项对照
├── 论文双栏格式说明.md              双栏排版参数与实现说明
├── 计算机工程与应用期刊格式要求.docx   期刊官方投稿模板与格式要求
├── 项目相关文献检索汇总_40篇.md / .docx
├── 参考文献_著录信息_crossref.json   40 条文献的出版信息与原文献英文题名
...
此外，**论文稿件（`论文初稿_*.md`、`论文初稿_*.docx`）与投稿信**属个人隐私材料，已由 `.gitignore` 排除，不在本仓库内。
