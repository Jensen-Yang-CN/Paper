...
针对服装表面破洞、污渍、褶皱三类微痕检测中"专用检测模型定位准但疑难样本漏检多、多模态大模型语义判断强但定位粗"的能力互补性，本仓库给出一套**推理期协同**方案及其完整实验证据、可运行系统和投稿材料。
...
```
Paper/
├── 论文初稿_v3.md / .docx        ★ 投稿版（按《计算机工程与应用》须知改造）
├── 论文初稿_v4_双栏.docx          双栏排版版
├── 论文初稿_v5_模板版.docx        期刊官方模板套用版
├── 服装微痕检测_..._论文执行方案.md   项目执行方案（含投稿须知逐条对照表）
├── 投稿信草稿.md / .docx
├── 参考文献_40篇.bib              参考文献（GB/T 7714 顺序编码制，共 40 条）
├── 项目相关文献检索汇总_40篇.md
├── 软著_源程序文档.docx            软件著作权源程序文档（等宽字体，前 30 页 + 后 30 页，每页 50 行）
│
├── md2docx.py                   ★ Markdown → DOCX 转换器（三线表 / 图题表题中英对照 / 图片按真实 DPI 限宽 8 cm）
├── md2docx_cea.py / _cea2.py      期刊模板专用转换脚本
...
- **稿件**：`论文初稿_v3.md`（投稿版），4 幅插图、7 张表格，均为黑白、宽度 < 8 cm、图字小 6 号宋体
- **作者**：杨骏杰，武汉工程大学 计算机科学与工程学院
- **状态**：已完成写作与格式改造，待补充作者简介信息后投稿
...
本仓库为个人研究项目，用于记录研究过程与投稿材料。论文尚在投稿阶段，结论以最终发表版本为准。
